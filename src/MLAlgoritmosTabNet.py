...
# Features to use (using 'all' equivalent from the paper/code context if possible, or a specific set)
# The user mentioned "all the features" in the context of the paper results for Table IV and V.
# Let's use the full set available in the scaled files.
featuresSel = ['x', 'y', 'z', 'smv', 'smvfil', 'ratio', 'HRR', 'hr', 'xfil', 'yfil', 'zfil']
# Note: 'smv' is repeated in the original code's comment, I'll stick to unique valid features.
# Based on MLAlgoritmos.py line 117: 'x', 'y', 'z', 'hr', 'smv', 'xfil', 'yfil', 'zfil', 'smvfil', 'ratio','HRR'
...
def run_tabnet():
    ...
    for train_index, test_index in logocv.split(X, y_encoded, groups):
        ...
        clf.fit(
            ...
            max_epochs=100, # Reduced for speed, can be increased
            patience=20,
            batch_size=1024, 
    ...
    print("\n" + "="*30)
    print(f"TabNet Results (All Features)")
